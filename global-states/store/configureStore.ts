...
const makeStore = ({ isServer }: { isServer: boolean }) => {
  ...
  const store = createStore(persistedReducer, {}, bindMiddleware([thunkMiddleware, loggerMiddleware]))
...
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
export const nextReduxWrapperTS = createWrapper<Record<string, never>>(makeStore, { debug: true })
